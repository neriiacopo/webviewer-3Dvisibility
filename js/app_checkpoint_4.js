...
function createXYZRGB(arr) {
  ...
  let threshold = filterSlider.noUiSlider.get();
  let treedist = treeSlider.noUiSlider.get();
  function filterVals(arr, limit, limitTree) {
    ...
    let limitsTree = [parseInt(limitTree[0]), parseInt(limitTree[1])];
    for (let i = 0; i < arr.length; i++) {
      if (
        parseInt(arr[i].viz100) >= limits[0] &&
        parseInt(arr[i].viz100) <= limits[1] &&
        parseInt(arr[i].treedist) >= limitsTree[0] &&
        parseInt(arr[i].treedist) <= limitsTree[1]
      ) {
        // positions
  ...
  filterVals(arr, threshold, treedist);

  filterSlider.noUiSlider.on("update", function (values) {
    threshold = [parseInt(values[0]), parseInt(values[1])];
    filterVals(arr, threshold, treedist);
  });
